...
static uint16_t          as5147_tx = 0xFFFF;   // NOP read command
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    // Latch angle from previous DMA result, then start new read
    motor.state.theta_mech_raw = (float)(as5147_rx & 0x3FFF)
...
int main(void)
{
  ...
  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_4, 441);
