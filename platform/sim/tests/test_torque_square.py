"""
Torque (current) control — i_q = ±0.4 A square wave at 5 Hz, i_d = 0.

FOC_MODE_TORQUE: the current PI loops regulate i_d and i_q independently.
Observe that i_q tracks the square wave reference while i_d stays near zero,
and that motor speed oscillates symmetrically in response to the alternating
torque command.
"""
...
def ref_fn(t):
    iq = IQ_AMP if (t * FREQ % 1.0) < 0.5 else -IQ_AMP
    return {'i_d_ref': 0.0, 'i_q_ref': iq}
...
log = sim.run(duration=0.5, mode=FOC_MODE_TORQUE, ref_fn=ref_fn)
...
fig.suptitle('Torque control — i_q = ±0.4 A square wave 5 Hz, i_d = 0')

axes[0].plot(t_ms, log['i_q_ref'], label='i_q_ref', linestyle='--', color='gray')
axes[0].plot(t_ms, log['i_q'],     label='i_q (measured)')
axes[0].set_ylabel('q-axis current (A)')
axes[0].legend()
axes[0].grid(True)

axes[1].plot(t_ms, log['i_d'])
axes[1].set_ylabel('d-axis current (A)')
axes[1].grid(True)
