...
-----
  ...
  2. Speed detail (last ZOOM_MS ms) — raw backward-difference omega,
     LPF-filtered estimate, and physics truth side-by-side.
...
DURATION     = 2.5        # s — total run (STEP_TIME + 0.5 s of steady state)
...
# -- 2. Speed detail: raw vs LPF vs truth (zoom) ----------------------------
ax   = axes[1]
sl   = slice(-n_zoom, None)
t_z  = t_ms[sl]
ax.plot(t_z, omega_raw[sl],             label='ω raw (backward diff, quantized)',
        color='tab:red',   alpha=0.5, linewidth=0.8)
ax.plot(t_z, log['omega_mech'][sl],     label='ω truth',
        color='tab:blue',  alpha=0.8)
ax.plot(t_z, log['omega_mech_est'][sl], label='ω est (LPF)',
        color='tab:orange', linewidth=1.5, linestyle='--')
ax.set_ylabel('Speed (rad/s)')
ax.set_xlabel('Time (ms)')
ax.legend(fontsize=8)
ax.grid(True)
ax.set_title(f'Speed detail — last {ZOOM_MS:.0f} ms  (quantization noise vs LPF)', fontsize=9)
